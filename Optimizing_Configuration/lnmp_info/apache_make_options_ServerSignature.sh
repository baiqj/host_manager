...
#查看"Apache_Make_ServerSignature"所在的行号

LINE_NUM=`grep  -n  "Apache_Make_ServerSignature"   $ENV_PATH  |  awk -F:  '{print $1}'`

#在"Apache_Make_ServerSignature"行之后添加一行

sed  -ie  "/Apache_Make_ServerSignature/a \'Apache_Make_ServerSignature\':\'$VALUE\'" $ENV_PATH

#删除原来的"Apache_Make_ServerSignature"行
