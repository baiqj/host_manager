...
#查看"Nginx_Rpm_Vhost_Conf"所在的行号
LINE_NUM=`grep  -n  "Nginx_Rpm_Vhost_Conf"   $ENV_PATH  |  awk -F:  '{print $1}'`

#在"Nginx_Rpm_Vhost_Conf"行之后添加一行
sed  -ie  "/Nginx_Rpm_Vhost_Conf/a \'Nginx_Rpm_Vhost_Conf\':`cat  cache.tmp`" $ENV_PATH

#删除原来的"Nginx_Rpm_Vhost_Conf"行
