...
declare  -i     CONF_LINE=0  VHOST_NUM=0     ZERO=0  ONE=1 
...
CONF_PATH=`/bin/rpm   -qc  httpd  |  grep  'httpd\.conf'`
...
CONF_PATH=`find   $RESULT_PATH/conf.list`
...
COUNT_4=`awk   '{print  $1}'    $RESULT_PATH/conf.list | grep  -w "NameVirtualHost" | wc  -l  |   awk   '{print  $1}' `
