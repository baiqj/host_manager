...
declare  -i  ONE=1   ZERO=0  FILE_LINE=0  CONF_LINE=0  VHOST_NUM=0
...
VER=`dpkg  -l |   grep   apache  |  awk  '{print  $2}'  |  grep   -v  "-"`
...
/usr/bin/updatedb
CONF_PATH=`/usr/bin/locate   $VER.conf  |  grep   "$WORK_DIR" |   grep  "conf$"`
