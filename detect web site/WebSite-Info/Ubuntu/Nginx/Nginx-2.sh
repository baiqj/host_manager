...
declare  -i   ZERO=0  ONE=1   CONF_LINE=0   VHOST_NUM=0    
...
COMMAND_PATH=`whereis  nginx |  awk  -F ":"  '{print  $2}'  |  awk  '{print  $1}'`

$COMMAND_PATH   -V  &>  $RESULT_PATH/config.list
 
COUNT_1=`grep   "configure.arguments:"   $RESULT_PATH/config.list  |  grep  "prefix"  |  wc  -l  |  awk  '{print  $1}'`

if  [ $COUNT_1 -eq  $ZERO ]
then
	WORK_DIR="/usr/local/nginx"
elif [ $COUNT_1 -eq  $ONE ]
then
	
	WORK_DIR=`grep  "configure.arguments:"   $RESULT_PATH/config.list |  awk  -F ":"  '{print  $2}'  |  awk  -F  "--prefix="   '{print  $2}'  | awk  '{print  $1}'
  `
fi
...
CONF_PATH=$WORK_DIR/conf/nginx.conf
...
do
	...
	echo   "$NAME" >>   $RESULT_PATH/Include_file.list
done
